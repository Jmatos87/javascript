...
var output = [ ]
    for(var i=0; i<objectsArray.length; i++ ){
           var fullName = objectsArray[i]
           output.push(fullName.first+ ' ' +fullName.last)
    }
   return output 
}
...
var getCounts = function (stringOfWords){
 ...
    
    var word = arrayOfWords[i]
     //as its found it has 1 value
    //if property is not on the object
        // create the property on the object and set its value to one
    //else (if property is on the object)
    //
        if(wordsObject.hasOwnProperty(word) === true) {
        wordsObject[word]+= 1}
        
        else{wordsObject[word] =1 }
    }
...
// Human explanation
// Create an object with attributes (legs, speak, and name)
// Legs attribute never change and === 4
// speak is a function that replaces the first letter of input with r
// dog name is a function that rewrites the name attribute of said dog.
...
var generateDog = function (input){
    ...
    this.speak = function(words){
        var takeApart = words.split(' ')
        var newSpeech = '' 
        for (var i = takeApart.length-1; i>=0; i--){
             var word = takeApart[i]
             var newWord = word.substring(1)
             var wordR = "r" + newWord 
             newSpeech = wordR + " " + newSpeech
            
        }
        return newSpeech.slice(0,newSpeech.length-1)
    }
...
//
//Human Explanation
//Create a function that only calls the first key in all the objects.
//Similar to movie titles

// tests
// ---


var pluck = function (objectList, key){
    
   var newArray = []
   for(var i=0; i<objectList.length;i++){
       var stoogeObject = objectList[i]
        newArray.push(stoogeObject[key])
